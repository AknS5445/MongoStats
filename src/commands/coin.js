...
module.exports = {
  ...
  execute: async (client, message, args, embed, prefix) => {
    ...
    if (args[0] === "ekle" || args[0] === "add") {
      ...
      if (!coinData || coinData && count > coinData.coin) return message.channel.send(embed.setDescription("Göndereceğin coin sayısı kendi coininden yüksek olamaz!"));
      ...
      message.channel.send(embed.setDescription(`${member.toString()} kullanıcısına başarıyla **${count}** coin gönderildi!`));
    }
  }
};
